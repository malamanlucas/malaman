...
<c:choose>
	<c:when test="${not empty carousels}">
		<c:forEach items="#{carousels}" var="i">
			<div class="painel-item txt14">
				<div class="p-img">
					<img src="<c:url value="/estatico/img/loader.gif" />"
						 class="img-lazyload"
						 data-original="<c:url value="/carousel/off/foto?id=${i.id}" />" />
				</div>
				<div class="p-dsc center">
					<input type="hidden" class="id" value="${i.id}" />
					${i.id}
					<sec:authorize access="hasRole('ROLE_GESTOR')">
						<a class="btn btn-md btn-primary submit-form" method="POST" url="<c:url value="/carousel/edit?id=${i.id}"/>">Editar</a>
	...
	<c:otherwise>
		<vl-tags:list_empty_message msg="Não há nenhum carousel cadastrado."/>
	</c:otherwise>
</c:choose>
