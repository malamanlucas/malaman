...
<c:choose>
	<c:when test="${not empty lanches}">
		<c:forEach items="#{lanches}" var="i">
			<div class="painel-item txt14 check-new check-enabled">
				...
				<div class="p-dsc center">
					...
					<sec:authorize access="hasRole('ROLE_GESTOR')">
						<a class="btn btn-md btn-primary submit-form" method="POST" url="<c:url value="/lanche/edit?id=${i.id}"/>">Editar</a>
	...
	<c:otherwise>
		<vl-tags:list_empty_message msg="Nenhum lanche cadastrado"/>
	</c:otherwise>
</c:choose>
